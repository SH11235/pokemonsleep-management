...
describe('BoostEventSelector', () => {
    it('renders correctly', () => {
        ...
        render(<BoostEventSelector boostEvent="none" setBoostEvent={mockSetBoostEvent} />);

        expect(screen.getByText('ブーストイベント')).toBeInTheDocument();
        Object.values(candyBoostMultipliers).forEach(({ label }) => {
    ...
    it('calls setBoostEvent when a radio button is selected', () => {
        ...
        expect(mockSetBoostEvent).toHaveBeenCalledWith(Object.keys(candyBoostMultipliers)[1]);
    });
});
